...
const UniversityStateManager = () => {
  ...
  const handleAddUniversity = () => {
    if (newUniversityName.trim() && selectedState) {
      addUniversity.mutate({ name: newUniversityName.trim(), state: selectedState });
    }
  };
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {states?.map((state) => (
                      <div key={state.id} className="flex items-center justify-between p-3 border rounded">
                        ...
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => toggleStateStatus.mutate({ id: state.id, is_active: !state.is_active })}
                          >
                ...
                <CardContent>
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {universities?.map((university) => (
                      <div key={university.id} className="flex items-center justify-between p-3 border rounded">
                        <div className="flex items-center gap-3">
                          <div>
                        ...
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => toggleUniversityStatus.mutate({ id: university.id, is_active: !university.is_active })}
                          >
                          ...
                          <Button
                            size="sm"
                            variant="destructive"
                            onClick={() => deleteUniversity.mutate(university.id)}
                          >
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
